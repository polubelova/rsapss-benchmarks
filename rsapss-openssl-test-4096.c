...
int main() {
  ...
  for (int j = 0; j < ROUNDS; j++) {
    hacl_sign(4096U, vectors[4].n, 24U, vectors[4].e, 4096U, vectors[4].d, 128U, vectors[4].msg, 0U, NULL, comp);
    res = res ^ comp[0];
  }
  ...
  for (int j = 0; j < ROUNDS; j++) {
    int r = hacl_verify(4096U, vectors[4].n, 24U, vectors[4].e, 128U, vectors[4].msg, 0U, comp);
    res = res ^ r;
  }
