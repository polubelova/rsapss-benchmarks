...
extern void Hacl_Bignum64_bn_mod_exp_ct_precompr2(
  uint32_t len,
  uint64_t *n,
  uint64_t *a,
  uint32_t bBits,
...
void modexp_hacl_bm(uint32_t len, uint8_t* cb, uint8_t* ab, uint8_t* bb, uint8_t* nb, uint8_t* r2b){
  ...
  uint64_t c[len / 8];
  Hacl_Bignum64_bn_mod_exp_ct_precompr2(len / 8, n, a, len * 8, b, r2, c);
